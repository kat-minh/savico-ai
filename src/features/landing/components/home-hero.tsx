    ...
    <section className='mx-auto grid w-full max-w-[90rem] items-center gap-10 px-4 py-16 lg:grid-cols-[9fr_10fr] lg:gap-10 lg:px-8 lg:py-24'>
      <div className='space-y-8'>
        <div className='space-y-5'>
          ...
          <p className='text-muted-foreground max-w-md text-lg text-pretty'>{t('subtitle')}</p>
        </div>
            ...
            return (
              <li key={promise} className='flex items-center gap-3'>
                ...
                <span className='flex flex-col leading-tight'>
                  <span className='text-base font-semibold'>{t(`promises.${promise}.title`)}</span>
                  <span className='text-muted-foreground text-sm'>{t(`promises.${promise}.hint`)}</span>
                </span>
              </li>
            )
          })}
