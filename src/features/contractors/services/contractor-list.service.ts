...
export interface ContractorCriteria {
  /** id loại công trình (`townhouse`, `villa`…) — khớp `Contractor.buildingTypes`. */
...
export function matchesCriteria(contractor: Contractor, criteria: ContractorCriteria): boolean {
  ...
  if (buildingTypeId && contractor.buildingTypes && !contractor.buildingTypes.includes(buildingTypeId)) return false
