import { DEFAULT_PACKAGE_TIER, FIELDS_BY_BUILDING_TYPE } from '../constants/design.constants'
...
/** Which conditional fields the chosen building type shows (mục III.2, ghi chú). */
export function visibleFields(buildingType: BuildingType | null) {
  if (!buildingType) return { floorCount: false, attic: false, architectureStyle: false }
...
export function applyBuildingTypeChange(input: DesignInput, buildingType: BuildingType): DesignInput {
  ...
  return {
    ...
    architectureStyle: fields.architectureStyle ? input.architectureStyle : null
  }
}

/** Field keys the submit button waits on, in the order they appear on screen. */
export type RequiredInputField = 'landPhotoUrl' | 'address' | 'buildingType' | 'floorCount' | 'hasAttic' | 'packageTier' | 'architectureStyle' | 'interiorStyle' // prettier-ignore
...
export function missingRequiredFields(input: DesignInput): RequiredInputField[] {
  ...
  if (fields.architectureStyle && !input.architectureStyle) missing.push('architectureStyle')
  if (!input.interiorStyle) missing.push('interiorStyle')
